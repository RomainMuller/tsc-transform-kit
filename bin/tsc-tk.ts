  ...
  if (watch) {
    ...
    project
      .once(BuildEvent.BeforeSolution, () => {
        clearScreen();
        console.log(`[\x1B[90m${new Date().toLocaleTimeString()}\x1B[0m] Starting compilation in watch mode...\n`);

        project.on(BuildEvent.BeforeSolution, () => {
          clearScreen();
          console.log(`[\x1B[90m${new Date().toLocaleTimeString()}\x1B[0m] File change detected. Starting incremental compilation...\n`);
        });
      })
      .on(BuildEvent.AfterSolution, (_, errorCount) => {
        console.log(`[\x1B[90m${new Date().toLocaleTimeString()}\x1B[0m] Found ${errorCount} errors. Watching for file changes.\n`);
      })
      .watch();
  } else {
    project.build();
