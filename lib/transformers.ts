...
export class Transformers {
  ...
  public forInvalidatedProject<T extends ts.BuilderProgram>(project: ts.InvalidatedProject<T>): ts.CustomTransformers {
    return {
      ...
      afterDeclarations: this.transformers.map(tx => customTransformerFactory(tx, TransformerPhase.AfterDeclarations)),
    };

    function customTransformerFactory(tx: Transformer, phase: TransformerPhase): ts.CustomTransformerFactory {
