...
describe('examples', () => {
  ...
  test('"basic" can be watched the UpcasingTransformer', async () => {
    ...
    return withTemporaryCopy(resolve(examplesDir, 'basic'), async (root) => {
      ...
      return expect(
        promisify(readFile)(resolve(root, 'dist', 'index.js'), {
          encoding: 'utf-8',
        }),
      )
        .resolves.toContain('MAIN')
          ...
          () =>
            new Promise((ok, ko) => {
              ...
              project.once(BuildEvent.AfterProject, () => {
                expect(
                  promisify(readFile)(resolve(root, 'dist', '_ignore.js'), {
                    encoding: 'utf-8',
                  }),
                )
              ...
              Promise.all([
                promisify(writeFile)(
                  resolve(root, 'dist', 'ignore.d.ts'),
                  'export const ignore = undefined;',
                ),
                promisify(writeFile)(
                  resolve(root, '_ignore.ts'),
                  'export const _ignore = undefined;',
                ),
              ]).then(() => null, ko);
            }),
